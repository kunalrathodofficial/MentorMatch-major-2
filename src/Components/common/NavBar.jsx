...
import logo from "../../assets/Logo/WhatsApp_Image_2024-04-29_at_00.47.47_a2cf4637-removebg-preview.png"
        ...
        <div className={` flex sm:relative bg-richblack-900 w-screen relative z-50 h-14 items-center justify-center border-b-[1px] border-b-richblack-700 translate-y-  transition-all duration-500`}>
            <div className='flex items-center justify-between w-11/12 max-w-maxContent'>
                <Link to='/' onClick={() => { dispatch(setProgress(100)) }}>
                    <img src={logo} width={160} alt="Mentor Match" height={42}></img>
                </Link>
                {/* mobile Navbar */}
                {
                    user && user?.accountType !== "Instructor" && (
                        <div className=' md:hidden'>
                            <Link to='/dashboard/cart' className='relative left-10' onClick={() => { dispatch(setProgress(100)) }} >
                                <div className=''>
                                    <TiShoppingCart className='w-8 h-8 fill-richblack-25' />
                                </div>
                                {
                                    totalItems > 0 && (
                ...
                <div className={`flex md:hidden  relative gap- flex-row ${token !== null && user?.accountType !== "Instructor" ? " -left-12" : ""}`}>
                    ...
                    <div ref={show} className='z-50 mobNav'>
                ...
                {/* Desktop Navbar */}
                <nav>
                    <ul className='flex-row hidden gap-5 gap-x-6 text-richblack-25 md:flex'>
                        {
                            NavbarLinks?.map((element, index) => (
                                <li key={index} >
                                    {
                                        element.title === "Catalog" ? (<div className='relative flex items-center cursor-pointer group'>
                                            <p>{element.title}</p>
                                            ...
                                            <div className='invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]'>
                                                <div className='absolute left-[50%] top-0 -z-10 h-6 w-6 translate-x-[80%] translate-y-[-40%] rotate-45 select-none rounded bg-richblack-5'></div>
                                                {
                                                    sublinks?.length < 0 ? (<div></div>) : (
                                                        sublinks?.map((element, index) => (
                                                            <Link to={`/catalog/${element?.name}`} key={index} className="py-4 pl-4 bg-transparent rounded-lg hover:bg-richblack-50" onClick={() => { dispatch(setProgress(30)) }}>
                                                                <p className=''>
                                                                    {element?.name}
                                                                </p>
                                                            </Link>
                        ...
                        <form onSubmit={handelSearch} className='relative flex items-center'>
                            ...
                            <HiSearch type='submit' id='searchicon' size={20} className="absolute cursor-pointer text-richblack-100 top-1 left-20" />
                        </form>
                    </ul>
                </nav>

                <div className='flex-row items-center hidden gap-5 md:flex'>
                    {
                        user && user?.accountType !== "Instructor" && (
