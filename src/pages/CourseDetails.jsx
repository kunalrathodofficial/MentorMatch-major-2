...
const CourseDetails = () => {
    ...
    if(!courseDetail) return <div className='flex items-center justify-center h-screen'>
  ...
  return (
    <div>
        <div className='mx-auto box-content px-4 lg:w-[1260px] lg:relative '>
            <div className='mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-center py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]'>
                <div className='relative block max-h-[30rem] lg:hidden'>
                    ...
                    <div className='z-30 flex flex-col justify-center gap-4 py-5 my-5 text-lg text-richblack-5'>  
                            <p className='text-4xl font-bold text-richblack-700 sm:text-[42px]'>{courseDetail?.courseName}</p>
                            <p className='text-richblack-500'>{courseDetail?.courseDescription}</p>
                            <div className='flex items-center gap-x-3'>
                        <span className='text-yellow-50'>{avgReviewCount || 0}</span>
                        <RatingStars Review_Count={avgReviewCount} />
                        <span className='hidden md:block md:text-xl text-richblack-500'>({courseDetail?.ratingAndReviews?.length} Reviews)</span>
                        {/* student enrolled */}
                        <span className='text-richblack-500'>{courseDetail?.studentsEnrolled?.length} students enrolled</span>
                    </div>
                    <div>
                        <p className='text-richblack-600' >Created By {courseDetail?.instructor?.firstName}  {courseDetail?.instructor?.lastName}</p>
                    </div>
                    <div className='flex flex-wrap gap-5 text-lg'>
                        <AiOutlineInfoCircle className='text-2xl text-richblack-400' />
                        <p className='text-richblack-500'>Created at &nbsp;    
                        ...
                        <p className='flex items-center gap-2 text-richblack-600'><BsGlobe className='text-lg text-richblack-500'/>English</p>
                    </div>
                    </div>
                    <div className='flex flex-col w-full gap-4 py-4 border-y border-y-richblack-500 lg:hidden'>
                        <p className='pb-4 space-x-3 text-3xl font-semibold text-richblack-600'>
                ...
                <div className='right-[1rem] top-[60px] mx-auto hidden min-h-[600px] w-1/3 max-w-[410px] translate-y-24 md:translate-y-0 lg:absolute  lg:block'>
                    <div className='flex flex-col gap-4 p-4 rounded-md bg-richblack-700 text-richblack-5'>
                        ...
                        <div className='px-4'>
                            ...
                            <div className='pt-6 pb-3 text-sm text-center text-richblack-25'>
                            ...
                            <div className='text-center'>
                                {/* copy url */}
                                <button className='flex items-center gap-2 py-6 mx-auto text-yellow-100' onClick={
                                    () => {
            ...
            <div className='mx-auto box-content px-4 text-start text-richblack-700 lg:w-[1260px]'>
                                        ...
                                        <div className='mt-5'>
                                            {
                                                item?.subSection?.map((subItem, subIndex) => (
                                                    <div key={subIndex} className='relative p-5 overflow-hidden border border-solid bg-richblack-900 border-richblack-600'>
                                                        <div className='flex items-center gap-2'>
                ...
                <p className='mb-10 text-sm text-richblack-700'>{courseDetail?.instructor?.additionalDetails?.about} </p>
            </div>
            ...
            <div className='mx-auto box-content px-4 text-start text-richblack-500 lg:w-[1260px]'>
                <div className='mx-auto max-w-maxContentTab lg:mx-0 xl:max-w-[990px]'>
                    <div className='p-3 my-8 border border-richblack-600 md:p-8'>
                        <p className='text-3xl font-semibold'>
                            Reviews
                        </p>
                        <div className='mt-5'>
                            <div className='flex items-center gap-4'>
                                <div className='flex items-center gap-2'>
                                    ...
                                    <span className='text-richblack-500'>({courseDetail?.ratingAndReviews?.length} ratings)</span>
                                    <span className='text-richblack-700'>|</span>
                                    <span className='text-richblack-500'> {courseDetail?.studentsEnrolled?.length} students</span>
                                    </div>
                                </div>
                                </div>
                                {
                                    courseDetail?.ratingAndReviews?.map((item, index) => (
                                        <div key={index} className='flex flex-col gap-3 my-4 mt-12 md:items-baseline ga'>
                                            <div className='flex items-center gap-2'>
                                                <img src={item?.user?.image} alt="user img" className='w-[30px] h-[30px] rounded-full object-cover'/>
                                                <div className='flex flex-col'>
                                                    <p className='font-semibold md:text-xl min-w-max'>{item?.user?.firstName} {item?.user?.lastName}</p>
                                                </div>
                                            </div>
